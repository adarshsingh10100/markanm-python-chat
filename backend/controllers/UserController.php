...
class UserController {
    ...
    public static function uploadAvatar(): void {
        ...
        if (!in_array($mime, ALLOWED_AVATAR_MIMES)) {
            jsonError('Invalid image format. Allowed formats: JPEG, PNG, WEBP, GIF.', 400);
        }
        ...
        $filename = 'avatar_' . $currentUser['id'] . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . strtolower($ext);
    ...
    public static function uploadBanner(): void {
        ...
        $ext = pathinfo($file['name'], PATHINFO_EXTENSION) ?: 'jpg';
        $filename = 'banner_' . $currentUser['id'] . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . strtolower($ext);
        $targetPath = UPLOAD_DIR . $filename;
